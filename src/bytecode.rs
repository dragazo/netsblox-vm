...
impl<'a> core::ops::Deref for CowStr<'a> {
    type Target = str;
...
pub(crate) enum Instruction<'a> {
    ...
    Call { pos: usize, tokens: CowStr<'a> },
    ...
    MakeClosure { pos: usize, params: usize, tokens: CowStr<'a> },
    ...
    SendNetworkMessage { tokens: CowStr<'a>, expect_reply: bool },
...
impl<'a> BinaryRead<'a> for CowStr<'a> {
    fn read(code: &'a [u8], data: &'a [u8], start: usize) -> (Self, usize) {
        let res = <&'a str as BinaryRead>::read(code, data, start);
        (CowStr::Borrowed(res.0), res.1)
    }
}
...
impl<'a> BinaryRead<'a> for Instruction<'a> {
    fn read(code: &'a [u8], data: &'a [u8], start: usize) -> (Self, usize) {
        ...
        match code[start] {
            ...
            32 => read_prefixed!(Instruction::ListCsv),
            33 => read_prefixed!(Instruction::ListColumns),
            34 => read_prefixed!(Instruction::ListLines),

            35 => read_prefixed!(Instruction::ListInsert),
            36 => read_prefixed!(Instruction::ListInsertLast),
            37 => read_prefixed!(Instruction::ListInsertRandom),

            38 => read_prefixed!(Instruction::ListGet),
            39 => read_prefixed!(Instruction::ListGetLast),
            40 => read_prefixed!(Instruction::ListGetRandom),

            41 => read_prefixed!(Instruction::ListAssign),
            42 => read_prefixed!(Instruction::ListAssignLast),
            43 => read_prefixed!(Instruction::ListAssignRandom),

            44 => read_prefixed!(Instruction::ListRemove),
            45 => read_prefixed!(Instruction::ListRemoveLast),
            46 => read_prefixed!(Instruction::ListRemoveAll),

            47 => read_prefixed!(Instruction::ListPopFirstOrElse {} : goto),

            48 => read_prefixed!(Instruction::Cmp { relation: Relation::Equal }),
            49 => read_prefixed!(Instruction::Cmp { relation: Relation::NotEqual }),
            50 => read_prefixed!(Instruction::Cmp { relation: Relation::Less }),
            ...
            57 => read_prefixed!(Instruction::BinaryOp { op: BinaryOp::Mul }),
            58 => read_prefixed!(Instruction::BinaryOp { op: BinaryOp::Div }),
            59 => read_prefixed!(Instruction::BinaryOp {} : op),

            60 => read_prefixed!(Instruction::VariadicOp { op: VariadicOp::Add, } : len),
            61 => read_prefixed!(Instruction::VariadicOp { op: VariadicOp::Mul, } : len),
            62 => read_prefixed!(Instruction::VariadicOp { op: VariadicOp::StrCat, } : len),
            63 => read_prefixed!(Instruction::VariadicOp { op: VariadicOp::MakeList, } : len),
            64 => read_prefixed!(Instruction::VariadicOp {} : op, len),

            65 => read_prefixed!(Instruction::UnaryOp { op: UnaryOp::Not }),
            66 => read_prefixed!(Instruction::UnaryOp { op: UnaryOp::Round }),
            67 => read_prefixed!(Instruction::UnaryOp {} : op),

            68 => read_prefixed!(Instruction::DeclareLocal {} : var),
            69 => read_prefixed!(Instruction::InitUpvar {} : var),
            70 => read_prefixed!(Instruction::Assign {} : var),

            71 => read_prefixed!(Instruction::BinaryOpAssign { op: BinaryOp::Add, } : var),
            72 => read_prefixed!(Instruction::BinaryOpAssign {} : var, op),

            73 => read_prefixed!(Instruction::Watcher {} : create, var),
            74 => read_prefixed!(Instruction::Pause),

            75 => read_prefixed!(Instruction::Jump {} : to),
            ...
            79 => read_prefixed!(Instruction::MakeClosure {} : pos, params, tokens),
...
impl BinaryWrite for Instruction<'_> {
    fn append(val: &Self, code: &mut Vec<u8>, data: &mut BinPool, relocate_info: &mut Vec<RelocateInfo>) {
        ...
        match val {
            ...
            Instruction::ListCsv => append_prefixed!(32),
            Instruction::ListColumns => append_prefixed!(33),
            Instruction::ListLines => append_prefixed!(34),

            Instruction::ListInsert => append_prefixed!(35),
            Instruction::ListInsertLast => append_prefixed!(36),
            Instruction::ListInsertRandom => append_prefixed!(37),

            Instruction::ListGet => append_prefixed!(38),
            Instruction::ListGetLast => append_prefixed!(39),
            Instruction::ListGetRandom => append_prefixed!(40),

            Instruction::ListAssign => append_prefixed!(41),
            Instruction::ListAssignLast => append_prefixed!(42),
            Instruction::ListAssignRandom => append_prefixed!(43),

            Instruction::ListRemove => append_prefixed!(44),
            Instruction::ListRemoveLast => append_prefixed!(45),
            Instruction::ListRemoveAll => append_prefixed!(46),

            Instruction::ListPopFirstOrElse { goto } => append_prefixed!(47: move goto),

            Instruction::Cmp { relation: Relation::Equal } => append_prefixed!(48),
            Instruction::Cmp { relation: Relation::NotEqual } => append_prefixed!(49),
            Instruction::Cmp { relation: Relation::Less } => append_prefixed!(50),
            ...
            Instruction::BinaryOp { op: BinaryOp::Mul } => append_prefixed!(57),
            Instruction::BinaryOp { op: BinaryOp::Div } => append_prefixed!(58),
            Instruction::BinaryOp { op } => append_prefixed!(59: op),

            Instruction::VariadicOp { op: VariadicOp::Add, len } => append_prefixed!(60: len),
            Instruction::VariadicOp { op: VariadicOp::Mul, len } => append_prefixed!(61: len),
            Instruction::VariadicOp { op: VariadicOp::StrCat, len } => append_prefixed!(62: len),
            Instruction::VariadicOp { op: VariadicOp::MakeList, len } => append_prefixed!(63: len),
            Instruction::VariadicOp { op, len } => append_prefixed!(64: op, len),

            Instruction::UnaryOp { op: UnaryOp::Not } => append_prefixed!(65),
            Instruction::UnaryOp { op: UnaryOp::Round } => append_prefixed!(66),
            Instruction::UnaryOp { op } => append_prefixed!(67: op),

            Instruction::DeclareLocal { var } => append_prefixed!(68: move str var),
            Instruction::InitUpvar { var } => append_prefixed!(69: move str var),
            Instruction::Assign { var } => append_prefixed!(70: move str var),

            Instruction::BinaryOpAssign { var, op: BinaryOp::Add } => append_prefixed!(71: move str var),
            Instruction::BinaryOpAssign { var, op } => append_prefixed!(72: move str var, op),

            Instruction::Watcher { create, var } => append_prefixed!(73: create, move str var),
            Instruction::Pause => append_prefixed!(74),

            Instruction::Jump { to } => append_prefixed!(75: move to),
            ...
            Instruction::MakeClosure { pos, params, tokens } => append_prefixed!(79: move pos, params, move str tokens),
